...
required_ui=['id="scheduleSubnav"','data-workflow-stage="shortage"','data-workflow-stage="available_to_issue"','data-workflow-stage="waiting_schedule"','data-workflow-stage="in_process"','switchWorkflowStageFromSidebar','updateScheduleSidebar']

if 'function workflowTabsHtml' in h:
    h=re.sub(r"    function workflowTabsHtml\(\)\{.*?\n    \}",'''    function workflowTabsHtml(){
...
if old in h and "currentSubTab = 'gantt';" not in h:
...
if MARK not in s:
    ...
    stage=r'''function detectWorkflowStage(row) {
...
function extractWorkflowRow'''
    ...
    s=detect.sub(stage,s,1)

    pb=re.compile(r"  if \(product\) \{.*?  if \(!\(capacity > 0\)\) capacity = 1000;\n  if \(!\(moldChange >= 0\)\) moldChange = 30;",re.S)
    pnew='''  const excelMaster = excelContext.products?.get(productCode) || null;
  if (product) {
  ...
  if (!(moldChange >= 0)) moldChange = 30;'''
    if not pb.search(s): raise SystemExit('product block not found')
    s=pb.sub(pnew,s,1)
    ...
    if up_old not in s: raise SystemExit('product upsert block not found')
    s=s.replace(up_old,up_new,1)

    pri=re.compile(r"function getSchedulePriority\(order\) \{.*?\n\}",re.S)
    pbody=r'''function getSchedulePriority(order) {
  ...
  return { level: 2, label: '三级：无交期后置', date: null, source: null };
    ...
    cbody=r'''function compareScheduleCandidates(a, b) {
  ...
  return 30;
    ...
    status='const updateOrderStatus = db.prepare("UPDATE orders SET status=\\'scheduled\\' WHERE id=?");'
    status_new='const updateOrderStatus = db.prepare("UPDATE orders SET status=\\'scheduled\\', workflow_production_progress=\\'已排待制\\' WHERE id=?");'
    if status not in s: raise SystemExit('auto schedule status statement not found')
    s=s.replace(status,status_new,1)
    s=MARK+'\n'+s
    SERVER.write_text(s,encoding='utf-8')

s=SERVER.read_text(encoding='utf-8'); h=PAGE.read_text(encoding='utf-8')
for x in ['V5.1.2-BUSINESS-LOGIC-VERIFIED',"source: 'workflow_expected_date_minus_1_day'","product?.machines","workflow_production_progress=\\'已排待制\\'"]:
    if x not in s: raise SystemExit('server validation failed: '+x)
for x in required_ui:
    if x not in h: raise SystemExit('UI validation failed: '+x)
if "currentSubTab = 'gantt';" not in h: raise SystemExit('schedule result view validation failed')
print('V5.1.2 business logic patch applied and verified')
